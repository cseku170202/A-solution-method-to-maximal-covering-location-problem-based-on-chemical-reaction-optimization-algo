...
void rand_node_gen(void) {
	...
	for (int i = 0; i < num_node; i++) {
		...
		Array_node[i].demand = rnd_demand_func(max_demand, 1); //demand is up to 50
	}
}
...
void dist_calc(void) {
	...
	get_coordinates_y.open("sjc324_coord_Y.txt");
	get_coordinates_x.open("sjc324_coord_X.txt");
	for (int i = 0; i < num_node; i++)
	{
...
int main() {
	...
	*/
	ofstream out2("sjc324_distance.txt");
